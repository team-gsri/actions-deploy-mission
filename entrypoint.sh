...
ssh "$SSH_USERNAME@$SSH_HOSTNAME" "Start-Process -Wait -Verb RunAs powershell '-File C:\Arma3\Stop-ArmaInstance.ps1 $ARMA_INSTANCE'"
scp "$LOCAL_PATH" "$SSH_USERNAME@$SSH_HOSTNAME:$REMOTE_PATH"
ssh "$SSH_USERNAME@$SSH_HOSTNAME" "Start-Process -Wait -Verb RunAs powershell '-Command Get-ScheduledTask $ARMA_INSTANCE | Start-ScheduledTask'"
